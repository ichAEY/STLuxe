...
for old,new in repls:
    if old not in s:
        raise SystemExit('expected v34 style not found: '+old[:90])
    s=s.replace(old,new,1)
old_small='.tn31-service-price{font-size:20px}}'
new_small='.tn31-service-price{font-size:17px}}'
if old_small not in s:
    raise SystemExit('v34 small-screen price style not found')
s=s.replace(old_small,new_small,1)
p.write_text(s)

idx=Path('index.html')
h=idx.read_text()
old='mobile-stluxe-final-v22.js?v=20260830-v34-seam-worklink-price'
new='mobile-stluxe-final-v22.js?v=20260830-v35-rollback-to-v33'
if old not in h:
    raise SystemExit('v34 cache key not found')
